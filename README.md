# Hi, I'm Alberto 

**Backend Developer in training** | Python · FastAPI · PostgreSQL | Cloud/DevOps

Computer Systems Engineering student (Mexico City). I build REST APIs with Python and I'm expanding my backend skills with Java/Spring Boot, while building my cloud infrastructure foundations: Docker, AWS, and CI/CD. My goal: solid backend today, Platform Engineering tomorrow.

---

##  Featured project

### [nihongo-srs](https://github.com/TU_USUARIO/nihongo-srs)
A spaced repetition (SRS) REST API for learning Japanese, built from scratch.

- **Stack:** Python · FastAPI · PostgreSQL · Docker
- **Focus:** OOP design, DRY principle, scheduling algorithm with `timedelta`
- **Practices:** professional Git workflow, Conventional Commits, automated tests
-  Under active development — every commit documents the process

---

## Stack

**Daily drivers**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

**Currently learning**
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white)
![Maven](https://img.shields.io/badge/Maven-C71A36?style=flat&logo=apachemaven&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonwebservices&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

**On the radar:** Kubernetes · Terraform · Go · Hibernate/JPA

---

---

## Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/albertognzga)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:albertognzga@gmail.com)

<!--
Optional: uncomment once you have more public repos with activity
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=TU_USUARIO&show_icons=true&theme=default&hide_border=true)
-->
